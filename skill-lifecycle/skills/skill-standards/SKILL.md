...
根据技能复杂度选择合适的文档组织方式，提升文档可维护性与可读性。

### 简单技能

**定义**: 功能单一、逻辑简单、代码量少的技能

**文档策略**: 直接在 `SKILL.md` 文件中完整描述
...
**定义**: 功能复杂、多模块协作、需要详细文档的技能

**文档策略**: 分层组织，模块化文档

**SKILL.md 定位**: 仅作为技能概览和索引
...
- **实现逻辑**: 详细的核心算法、业务逻辑（`references/implementation.md`）
- **参数定义**: 完整的 API 参数、配置项说明（`references/api-reference.md`）
- **使用示例**: 多场景示例、最佳实践（`references/examples.md`）
- **架构设计**: 系统架构、数据流图（`references/architecture.md`）
- **其他**: 按需创建其他专题文档
...
```
data-pipeline/
├── SKILL.md              # 概览：功能、场景、流程、索引
├── references/
│   ├── implementation.md # 详细实现逻辑
│   ├── api-reference.md  # 完整参数定义
│   ├── examples.md       # 多场景示例
...
| 正文行数   | < 300 行         | ≥ 300 行           |
| 功能数量   | 单一功能         | 多个子功能/模块    |
| 配置复杂度 | 无配置或少量配置 | 复杂配置、多参数   |
| 依赖关系   | 无依赖或简单依赖 | 多依赖、需要编排   |
| 文档需求   | 一页说明即可     | 需要分章节详细说明 |
...
- ✅ 所有内容在一个文件中，便于快速查阅
- ✅ 结构清晰，包含必需章节（目标、步骤、示例）
- ✅ 示例完整可复制，开箱即用

**复杂技能**:

- ✅ SKILL.md 能够在 5 分钟内让读者理解技能全貌
- ✅ 子文档结构清晰，链接有效
- ✅ 概览与详情分离，便于维护和更新
- ✅ 支持多人协作，不同模块可独立维护
...
- **灵活性**: 根据实际复杂度调整，不必严格遵循行数限制
- **渐进式**: 技能演进时，可从简单文档逐步拆分为复杂文档结构
- **一致性**: 同一项目中的技能文档结构应尽量保持一致
- **可维护性**: 文档结构应便于后续更新和维护
...
3. **禁止内容**: 不包含 README.md、tmp/、.cache/、**pycache**/ 等临时文件
4. **层级限制**: 最多两层
